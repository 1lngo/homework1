...
- **分析与图表导出**：年化指标、相关性热力图、收益分布 KDE、滚动波动率（HTML + CSV）。
- **预测与优化**：
  - Naive：近 30 日均值预测。
  - LightGBM：滞后项 + 滚动统计特征，固定测试集评估。
  - 组合优化：Monte Carlo（最大夏普 / 最小波动），并给出“低/中/高”风险偏好对应权重。
- **交互网页**：Streamlit 展示图表、推导过程、权重饼图与权重数组格式输出。
...
- `app.py`：Streamlit 应用入口（读取 `data.json` / `models.json`）。
...
```powershell
cd c:\Users\zaoji\Documents\trae_projects\1902
